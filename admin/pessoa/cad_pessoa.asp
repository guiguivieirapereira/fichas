...
<%
	...
	if len(id) > 0 then
		...
		if not objrs.eof then
            ...
            fichas_pendentes = 0
            set objrsFP = objConn.execute("select id from "&prefixo&"pessoa where id_indicacao = '"&id&"' and (length(nome) = 0 or nome is null)")
            if not objrsFP.eof then
                fichas_pendentes = 1
            end if
		end if
	end if	
                                        ...
                                        <div class="">
                                            ...
                                            <form action="cad_pessoa2.asp" method="post" name="frmcad" target="cadastro">
                                                ...
                                                <div class="box-footer">
                                                    ...
                                                    <button type="button" class="btn btn-info" onclick="window.open('imprimir.asp?id=<%=id%>', 'imprimir_fichas<%=id%>', 'width=800, height=500');">Imprimir</button>
						                    <%
                                                if len(id) > 0 then
                                            ...
                                            <%
                                                    ...
                                                    if fichas_pendentes = 1 then
                                            %>
                                                        <button type="button" class="btn btn-info" onclick="window.open('imprimir_fichas.asp?id=<%=id%>', 'imprimir_fichas<%=id%>', 'width=800, height=500');">Imprimir fichas</button>
                                            <%
                                                    end if
                                            %>
...
<script type="text/javascript">
    ...
    $(document).on('submit', 'form', function () {
        ...
        $(this).find('button[type=submit]').append('<i class="fa fa-spinner fa-pulse fa-fw"></i>')

        setTimeout(liberaBotao, 10000);
    });

    function liberaBotao() {
        $(document).find('button[type=submit]').removeAttr('disabled');
        $(document).find('button[type=submit]').find('.fa-spinner').remove();
    }
</script>
<%
	closedb()
