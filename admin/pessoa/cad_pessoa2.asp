...
<%
    ...
    if len(id) > 0 then
        ...
        if len(num_fichas_ant) = 0 then
            num_fichas_ant = 0
        end if

        if cint(num_fichas) < cint(num_fichas_ant) then
            erro = 1
            msg = msg&"Não é permitido diminuir o número de fichas!\n"
        end if
    end if
    ...
    if erro = 0 then
		if len(id) > 0 then
			strq = "update "&prefixo&"pessoa set nome='"&nome&"', cep='"&cep&"', rua='"&rua&"', numero='"&numero&"', complemento='"&complemento&"', bairro='"&bairro&"', "&_
                   ...
                   " observacao='"&observacao&"', senha='"&senha&"', dt_update = now() where id = "&id&""
...
<%
            ...
            strq = "insert into "&prefixo&"pessoa(nome, cep, rua, numero, complemento, bairro, cidade, estado, num_fichas, zona, secao, telefone1, telefone2, email, "&_
                   " citru, banner, material, reuniao, observacao, senha, datahora) values('"&nome&"', '"&cep&"', '"&rua&"', '"&numero&"', '"&complemento&"', '"&bairro&"', "&_
                   " '"&cidade&"', '"&estado&"', '"&num_fichas&"', '"&zona&"', '"&secao&"', '"&telefone1&"', '"&telefone2&"', '"&email&"', '"&citru&"', '"&banner&"', "&_
                   " '"&material&"', '"&reuniao&"', '"&observacao&"', '"&senha&"', now())"
			objConn.execute(strq)
			...
			if not objrs.eof then
                for i = 1 to cint(num_fichas)
                    objConn.execute("insert into "&prefixo&"pessoa(id_indicacao, nivel, num, datahora) values('"&objrs("id")&"', "&objrs("nivel")&" + 1, '"&i&"', now())")
                next
%>
	            <script type="text/javascript">
                    {
